...
export default function TugasPendahuluanDetailPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        praktikumId,
        pertemuanId,
        userId: "1", // Hardcoded for now, replace with actual user ID later
      ...
      // Submit to API - Pastikan endpoint ini sudah benar dan tersedia
      ...
      if (contentType && contentType.indexOf("application/json") !== -1) {
        ...
        console.error("Server tidak mengembalikan JSON:", textResponse.substring(0, 150) + "...");
  ...
  return (
    <>
      ...
      {/* Page Header - will appear in the blue background area of the layout */}
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-white font-semibold text-2xl md:text-3xl tracking-tight">
      ...
      {/* Loading State */}
      {loading && (
        <div className="bg-white rounded-xl shadow-lg p-8 flex justify-center items-center py-16">
          <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-[#0267FE]"></div>
        </div>
      )}
      ...
      {!loading && soalList.length > 0 ? (
        <div className="bg-white rounded-xl shadow-lg border border-gray-100">
          ...
          <div className="px-6 md:px-8 py-6 bg-gray-50 rounded-b-xl flex justify-end">
            <Button 
              className="bg-[#0267FE] hover:bg-blue-700 text-white px-6 py-2 rounded-lg shadow-md hover:shadow-lg transition-all flex items-center space-x-2"
