...
const Layout = ({ children }: { children: React.ReactNode }) => {
  ...
  // Set role ketika session sudah siap
  useEffect(() => {
  ...
  // Pilih warna banner berdasarkan role
  const bgClass =
    role === "asisten"
      ? "bg-gradient-to-br from-purple-600 via-purple-700 to-indigo-800"
      : "bg-gradient-to-br from-[#0267FE] to-blue-700";
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <main className="flex-1">
        <div className="relative">
          <div className={`absolute top-0 left-0 w-full h-72 ${bgClass} z-0 transition-colors duration-300`}></div>
          <div className="relative z-10 pt-4">
            {/* Header */}
            <Navbar />
            {/* Content */}
